...
function pdf( x, options ) {
	...
	if ( isNumber( x ) ) {
		return pdf1( x, opts.k, opts.lambda );
	}
	if ( isMatrixLike( x ) ) {
		if ( opts.copy !== false ) {
		...
		return pdf5( out, x, opts.k, opts.lambda );
	}
	if ( isTypedArrayLike( x ) ) {
		...
		return pdf6( out, x, opts.k, opts.lambda );
	}
	if ( isArrayLike( x ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return pdf4( x, opts.k, opts.lambda, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return pdf3( out, x, opts.k, opts.lambda, opts.accessor );
		}
		return pdf2( out, x, opts.k, opts.lambda );
	}
	return NaN;
} // end FUNCTION pdf()
